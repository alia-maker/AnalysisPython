...
def plotHoltWinters(data):
    ...
    # print('Lower inf:', list(model.LowerBond).count(np.inf))
    # print('Upper inf:', list(model.UpperBond).count(np.inf))
    for i in range(len(data["UpperBond"])):
        # print(data.values[i], model.LowerBond[i])
        if data["Real"][i] != np.inf:
            if data["Real"][i] < data["LowerBond"][i]:
                Anomalies[i] = data["Real"][i]
            if data["Real"][i] > data["UpperBond"][i]:
                Anomalies[i] = data["Real"][i]
    ...
    # plt.plot(data.index, data["Real"], label="Actual")
    # plt.plot(data.index, data["Model"], label="Model")
    # plt.plot(data.index, data["UpperBond"], "r--", alpha=0.5, label="Up/Low confidence")
    # plt.plot(data.index, data["LowerBond"], "r--", alpha=0.5)
    # # if model.UpperBond>model.LowerBond:
    #
    # plt.fill_between(x=data.index, y1=data["UpperBond"], y2=data["LowerBond"], alpha=0.2, color="grey")
    # #
    # # plt.plot(data.values, label="Actual", c='green')
    # plt.plot(data.index, Anomalies, "o", markersize=6, label="Anomalies", c='red')
